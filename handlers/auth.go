...
func LoginHandler(authClient *auth.Client) gin.HandlerFunc {
...
func CallbackHandler(authClient *auth.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		session := sessions.Default(c)
		...
		token, err := authClient.ExchangeCodeForToken(code, verifier.(string))
		if err != nil {
...
func RefreshTokenHandler(authClient *auth.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		session := sessions.Default(c)
		...
		token, err := authClient.RefreshAccessToken(refreshToken.(string))
		if err != nil {
...
func TokenMiddleware(authClient *auth.Client) gin.HandlerFunc {
	return func(c *gin.Context) {
		session := sessions.Default(c)
